...
export function getApiKeyFromHeaders(req: NextRequest, provider: string = "gemini") {
  const cleanKey = (key: string | null) => {
    if (!key) return null;
    const trimmed = key.trim();
    if (trimmed === "" || trimmed === "null" || trimmed === "undefined" || trimmed === "default") return null;
    return trimmed;
  };
  ...
  // 2. Final fallback to environment variables if still null
